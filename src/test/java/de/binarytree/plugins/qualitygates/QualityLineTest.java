...
public class QualityLineTest {
	...
	class MockQualityLine extends QualityLine {
		...
		GateEvaluator evaluatorMock; 
		public MockQualityLine(String name, Collection<Gate> gates)
				throws IOException {
			super(name, gates);
			this.evaluatorMock = mock(GateEvaluator.class); ; 
		}
		...
		public void verifyEvaluatorMock(){
			Mockito.verify(this.evaluatorMock).evaluate(any(AbstractBuild.class), any(Launcher.class), any(BuildListener.class)); 
		}
